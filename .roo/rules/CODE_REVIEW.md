...
- [ ] Обработчики событий находятся в одном месте (родитель или child, но не оба)
- [ ] Если child вызывает callback, родитель управляет состоянием и рефетч-логикой
- [ ] Используется переиспользуемый компонент ConfirmModal для всех подтверждений удаления
- [ ] Не используется window.confirm в UI-компонентах
