 ...
 * - Клик по карточке → переход на /dashboard/comms/chats/:chatId
 * - Чаты фильтруются: минимум 2 символа для поиска
 *
 * @covers AC-R21-1 (B-022-T2): Перенос компонента в features/comms
 * @task B-022-T2 (R-21)
 */
'use client';
...
export function ChatList(): React.JSX.Element {
  ...
  const handleCardClick = useCallback(
    (chatId: string) => {
      router.push(`/dashboard/comms/chats/${chatId}`);
    },
    [router],
  );
  ...
  const handleCreateChat = useCallback(() => {
    router.push('/dashboard/comms/chats/new');
  }, [router]);
